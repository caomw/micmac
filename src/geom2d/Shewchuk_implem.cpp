...
/*   the procedure triangulate(xxxx) that results.                               */
...
/* the unpardonable sin of including stdlib.h.  Hence, malloc(xxxx), free(xxxx), and */
/* exit(xxxx) may or may not already be defined at this point.  I declare these  */
...
struct memorypool;
extern void poolrestart(struct memorypool *);
#ifndef TRILIBRARY
char *readline();
char *findfield();
...
REAL incircleadapt(point pa, point pb, point pc, point pd, REAL permanent)
{
  ...
  axtbclen=aytbclen=bxtcalen=bytcalen=cxtablen=cytablen=0; /*warning init */
...
enum locateresult dt2d_locate( point searchpoint, struct triedge *searchtri)
{
  VOID **sampleblock;
  triangle *firsttri;
...
)
{
  ...
  struct triedge botlcasing, botrcasing;
  struct triedge toplcasing, toprcasing;
toprcasing.tri = 0; // Warning non init
toprcasing.orient = 0; // Warning non init
  ...
  if (splitedge == (struct edge *) NULL) {
    ...
    if (searchtri->tri == (triangle *) NULL) {
      ...
      intersect = dt2d_locate(insertpoint, &horiz);
    } else {
...
/*
int reconstruct(trianglelist, 
                triangleattriblist, trianglearealist, elements,
...

{
  ...
  if (vararea) {
    ...
    areafile = ElFopen(areafilename, "r");
     ...
     )
{
  ...
  point newpoint;
  enum insertsiteresult success;
  enum finddirectionresult collinear; GccUse(collinear);
  REAL ex, ey;
  REAL tx, ty;
  REAL etx, ety;
  REAL split, denom;
...
int scoutsegment( struct triedge *searchtri, point endpoint2, int newmark)
{
  ...
  point leftpoint, rightpoint;
  point endpoint1; GccUse(endpoint1);
...
void insertsegment( point endpoint1, point endpoint2, int newmark)
{
  ...
  if (checkpoint != endpoint1) {
    ...
    if (dt2d_locate(endpoint1, &searchtri1) != ONVERTEX) {
      printf(
  ...
  if (checkpoint != endpoint2) {
    ...
    if (dt2d_locate(endpoint2, &searchtri2) != ONVERTEX) {
      printf(
        "Internal error in insertsegment():  Unable to locate PSLG point\n");
      printf("  (%.12g, %.12g) in triangulation.\n",
             endpoint2[0], endpoint2[1]);
     ...
     )
{
  ...
  regiontris = (struct triedge  *) NULL; /* warning init */
    ...
    for (i = 0; i < 2 * holes; i += 2) {
      ...
      if ((holelist[i] >= xmin) && (holelist[i] <= xmax)
        ...
        if (counterclockwise(searchorg, searchdest, &holelist[i]) > 0.0) {
          ...
          intersect = dt2d_locate(&holelist[i], &searchtri);
  ...
  /*   dt2d_locate() won't work when the triangulation is no longer convex.       */
  ...
  if (regions > 0) {
    ...
    for (i = 0; i < regions; i++) {
      ...
      if ((regionlist[4 * i] >= xmin) && (regionlist[4 * i] <= xmax) &&
        ...
        /*   edge; otherwise, dt2d_locate() will falsely report that the     */
        ...
        if (counterclockwise(searchorg, searchdest, &regionlist[4 * i]) >
          ...
          intersect = dt2d_locate(&regionlist[4 * i], &searchtri);
          if ((intersect != OUTSIDE) && (!infected(searchtri))) {
...
FILE **polyfile;
{
  ...
  if (poly) {
    ...
    *polyfile = ElFopen(polyfilename, "r");
  ...
  if (readnodefile) {
    ...
    infile = ElFopen(innodefilename, "r");
    if (infile == (FILE *) NULL) {
      printf("  Error:  Cannot access file %s.\n", innodefilename);
      exit(1);
    }
    ...
    )
{
  ...
  outfile = ElFopen(nodefilename, "w");
    ...
    )
{
  ...
  outfile = ElFopen(polyfilename, "w");
  if (outfile == (FILE *) NULL) {
    printf("  Error:  Cannot create file %s.\n", polyfilename);
    exit(1);
  }
...

{
  ...
  outfile = ElFopen(edgefilename, "w");
  if (outfile == (FILE *) NULL) {
    printf("  Error:  Cannot create file %s.\n", edgefilename);
    exit(1);
  }
...

{
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  outfile = ElFopen(vnodefilename, "w");
  if (outfile == (FILE *) NULL) {
    printf("  Error:  Cannot create file %s.\n", vnodefilename);
    exit(1);
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  outfile = ElFopen(vedgefilename, "w");
  if (outfile == (FILE *) NULL) {
    printf("  Error:  Cannot create file %s.\n", vedgefilename);
    exit(1);
  }
...
void writeneighbors( int **neighborlist)
{
  ...
  outfile = ElFopen(neighborfilename, "w");
  if (outfile == (FILE *) NULL) {
    printf("  Error:  Cannot create file %s.\n", neighborfilename);
    exit(1);
  }
