...
class cHarrisOPB  : public Simple_OPBuf1<double,double>
{
   public :
...
void  cHarrisOPB::calc_buf ( REAL ** out, REAL *** in)
{
    REAL** i0 = in[0];
    REAL*  o0 = out[0];
    ...
    for (INT x=x0() ;  x<x1() ; x++)
    {
	  ...
	  double aDivG = pow(g2,mExpGrad/2.0);

          o0[x] =     ( c_xx * gy * gy - 2 * c_xy * gx * gy + c_yy * gx * gx) / aDivG;
        }
   }
}
